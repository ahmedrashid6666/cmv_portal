    ...
    return (
        <AuthenticatedLayout header="Bank Accounts">
            ...
            <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <Stat label="Fees & Expenses Paid Out" value={money(totals.customs_paid + totals.gov_paid + totals.other_paid + totals.office_expenses_paid + totals.ledger_paid, 'AED')} accent="text-accent-red" />
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b text-left text-xs uppercase text-slate-500">
                                ...
                                <th className="py-2 pr-4 text-right">Other Paid</th>
                                <th className="py-2 pr-4 text-right">Office Expenses</th>
                                <th className="py-2 pr-4 text-right">Bulk Paid</th>
                                <th className="py-2 pr-4 text-right">Balance</th>
                                <th className="py-2"></th>
                            </tr>
                        </thead>
                        <tbody>
                            {banks.length === 0 && <tr><td colSpan="11" className="py-8 text-center text-slate-400">No bank accounts yet. Add them under Master Data → Banks.</td></tr>}
                            {banks.map((b) => (
                                <tr key={b.id} className="border-b last:border-0 hover:bg-slate-200">
                                    ...
                                    <td className="py-2 pr-4 text-right tabular-nums text-emerald-700">{(b.sale_receipts + b.credit_received) ? '+' + money(b.sale_receipts + b.credit_received, 'AED') : '—'}</td>
                                    ...
                                    <td className="py-2 pr-4 text-right tabular-nums text-accent-red">{b.other_paid ? '−' + money(b.other_paid, 'AED') : '—'}</td>
                                    ...
                                    <td className="py-2 pr-4 text-right tabular-nums text-accent-red">{b.ledger_paid ? '−' + money(b.ledger_paid, 'AED') : '—'}</td>
                        ...
                        {banks.length > 0 && (
                            <tfoot>
                                <tr className="border-t-2 font-semibold text-navy-900">
                                    ...
                                    <td className="py-2 pr-4 text-right tabular-nums text-emerald-700">+{money(totals.sale_receipts + totals.credit_received, 'AED')}</td>
                                    ...
                                    <td className="py-2 pr-4 text-right tabular-nums text-accent-red">−{money(totals.other_paid, 'AED')}</td>
                                    <td className="py-2 pr-4 text-right tabular-nums text-accent-red">−{money(totals.office_expenses_paid, 'AED')}</td>
                                    <td className="py-2 pr-4 text-right tabular-nums text-accent-red">−{money(totals.ledger_paid, 'AED')}</td>
